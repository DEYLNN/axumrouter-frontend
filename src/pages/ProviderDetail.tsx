...
function SectionCard({
  ...
  return (
    <div className="brutal-card overflow-hidden">
      <div className="px-4 sm:px-5 py-3 sm:py-3.5 border-b-2 border-[#111111] flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span
            className="w-2.5 h-2.5 border-2 border-[#111111] shrink-0"
            style={{ backgroundColor: accent }}
          />
          <h2 className="heading-brutal text-sm uppercase tracking-tight">{title}</h2>
          {count !== undefined && (
            <span className="text-[10px] mono-brutal px-2 py-0.5 rounded-full border-2 border-[#111111] font-bold"
  ...
  return (
    <button
      ...
      className={`w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-lg border-2 transition-all disabled:opacity-40 ${
        active
          ? 'text-[#111111]'
          : 'text-gray-400 bg-white border-transparent hover:border-[#111111]'
      }`}
  ...
  return (
    <div className="brutal-card p-3 sm:p-4 hover:-translate-y-1 transition-all">
      <div className="flex items-start justify-between mb-2 sm:mb-3">
        <div
          className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center border-2 border-[#111111] shrink-0"
      ...
      <div className="text-[9px] sm:text-[10px] mono-brutal text-gray-500 mt-1.5 sm:mt-2 uppercase tracking-wider font-bold">
        {label}
      </div>
    </div>
  )
...
export default function ProviderDetail() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      {/* ─── HERO HEADER ─── */}
      <div>
        <Link to="/admin/providers" className="inline-flex items-center gap-1.5 text-xs mono-brutal text-gray-500 hover:text-[#111111] transition-colors mb-4 font-bold">
        ...
        {/* Hero header — responsive */}
          <div className="brutal-card overflow-hidden">
            {/* accent stripe */}
            <div className="h-1.5 border-b-2 border-[#111111]" style={{ backgroundColor: accent }} />
            <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4 relative">
              {/* icon */}
              <div
                className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl flex items-center justify-center border-2 border-[#111111] shrink-0 shadow-[2px_2px_0px_0px_#111111] sm:shadow-[3px_3px_0px_0px_#111111] lg:shadow-[4px_4px_0px_0px_#111111]"
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                  ...
                  <span
                    className="text-[10px] mono-brutal px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full border-2 border-[#111111] font-bold"
                ...
                <div className="flex items-center gap-2 sm:gap-2.5 mt-2 flex-wrap">
                  <span
                    className="text-[9px] sm:text-[10px] mono-brutal px-2 sm:px-2.5 py-0.5 rounded-full border-2 border-[#111111] font-bold"
                    style={{ backgroundColor: C.warning }}
                  >
                    {label.toUpperCase()}
                  </span>
                  <span className="text-[10px] mono-brutal text-gray-500">
                    <b className="text-[#111111]">{data.models?.length || 0}</b> models
                  </span>
                  <span className="text-gray-400">·</span>
                  <span className="text-[10px] mono-brutal text-gray-500">
                    <b className="text-[#111111]">{data.capabilities?.length || 0}</b> capabilities
                  </span>
                </div>
                {data.base_url && (
                  <div className="mt-2">
                    <span className="text-[9px] sm:text-[10px] mono-brutal text-gray-500 truncate max-w-full sm:max-w-[320px] inline-block bg-[#f0f0f0] px-2 py-0.5 rounded border-2 border-[#111111]" title={data.base_url}>
                      {data.base_url}
                    </span>
                  </div>
                )}
              ...
              {isCustom && (
                <button onClick={async () => {
                  ...
                  className="self-end sm:self-start w-9 h-9 flex items-center justify-center rounded-lg bg-white border-2 border-[#ff6b5e] text-[#ff6b5e] hover:bg-[#ff6b5e] hover:text-white transition-colors shrink-0"
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        ...
        <StatCard
          ...
          color={data.locked_keys > 0 ? C.danger : '#e5e5e5'}
        ...
        <SectionCard accent={C.accent} title="Capabilities" count={data.capabilities.length}>
          <div className="flex flex-wrap gap-2">
            {data.capabilities.map(c => (
              <span key={c}
                className="text-[10px] mono-brutal px-3 py-1.5 rounded-full border-2 border-[#111111] font-bold hover:-translate-y-0.5 transition-all"
      ...
      <SectionCard
        ...
        action={
          <button onClick={() => setShowAddModel(true)}
            className="brutal-btn bg-[#3ddc97] text-[#111111] px-3 py-1 text-[11px] font-bold">
            + Add Model
          </button>
        }
      >
        <div className="max-h-80 overflow-y-auto">
          <div className="divide-y-2 divide-[#111111]">
            {data.models.map(m => (
              <div key={m.id} className="px-4 sm:px-5 py-2.5 sm:py-3 flex items-center justify-between hover:bg-[#fdf9f0] transition-colors group">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 border-2 border-[#111111] ${m.available && !m.blocked ? 'bg-[#3ddc97]' : 'bg-gray-300'}`} />
                  <div className="min-w-0">
                    <div className="text-[11px] sm:text-xs font-bold text-[#111111] truncate">{m.name}</div>
                    <div className="text-[9px] mono-brutal text-gray-500 truncate mt-0.5">{m.id}</div>
                  </div>
                  {m.context_length && (
                    <span className="hidden sm:inline-block text-[9px] mono-brutal text-gray-600 bg-[#f0f0f0] px-2 py-0.5 rounded-full border-2 border-[#111111] shrink-0 ml-2 font-bold">
      ...
      >
        {data.keys.length === 0 ? (
          <div className="px-5 py-8 text-center text-xs mono-brutal text-gray-500">No keys configured</div>
        ) : (
          <div className="max-h-80 overflow-y-auto">
            <div className="divide-y-2 divide-[#111111]">
              {data.keys.map(k => (
                <div key={k.id} className="px-4 sm:px-5 py-2.5 sm:py-3 flex items-center justify-between group hover:bg-[#fdf9f0] transition-colors">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`w-2 h-2 rounded-full flex-shrink-0 border-2 border-[#111111] ${k.is_locked ? 'bg-[#ff6b5e]' : 'bg-[#3ddc97]'}`}
                        title={k.is_locked ? `Locked${k.locked_reason ? ': ' + k.locked_reason : ''}` : 'Active'} />
                      <span className="text-xs mono-brutal font-bold text-[#111111] truncate">{k.label || k.id}</span>
                    ...
                    <div className="text-[10px] mono-brutal text-gray-500 truncate mt-1">
                      <span className="bg-[#f0f0f0] px-1.5 py-0.5 rounded border border-[#111111]">{k.key_type || 'apikey'}</span>
                      <code className="text-gray-500 ml-1.5">{k.masked}</code>
                    </div>
                  </div>
        ...
        {/* Add Key */}
        {data.type !== 'oauth' && (
          <div className="px-4 sm:px-5 py-3 sm:py-4 border-t-2 border-[#111111]">
            <button onClick={() => ctx.setShowAddModal(true)}
              className="brutal-btn bg-[#c8a2ff] text-[#111111] px-3 sm:px-3.5 py-1.5 text-xs font-bold">
              + Add Key
            </button>
          </div>
        )}
          ...
          {keyFormConfig[data.id] ? (
            <div className="space-y-3">
              {keyFormConfig[data.id].fields.map(f => (
                <div key={f.key}>
                  <label className="text-[10px] mono-brutal text-gray-500 mb-1 block font-bold uppercase">{f.label}</label>
                  <input type="text" value={ctx.keyFields[f.key] || ''} onChange={e => ctx.setKeyFields(p => ({ ...p, [f.key]: e.target.value }))}
                    placeholder={f.placeholder}
                    className="w-full px-3 py-2 border-2 border-[#111111] rounded-lg text-xs mono-brutal bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#ff3d81]" />
                </div>
              ))}
            </div>
          ) : (
            <textarea value={ctx.newKeyValue} onChange={e => ctx.setNewKeyValue(e.target.value)}
              ...
              className="w-full px-3 py-2 border-2 border-[#111111] rounded-lg text-xs mono-brutal bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#ff3d81] h-24 resize-none" />
          )}
          <div className="flex items-center justify-end gap-2 mt-4">
            <button onClick={() => ctx.setShowAddModal(false)}
              className="brutal-btn bg-white text-[#111111] px-3 py-1.5 text-xs font-bold">Cancel</button>
            <button onClick={ctx.handleAddKey} disabled={ctx.adding}
              className="brutal-btn bg-[#3ddc97] text-[#111111] px-4 py-1.5 text-xs font-bold disabled:opacity-40">
      ...
      {testResult && (
        <Modal open={true} onClose={() => ctx.setTestResult(null)}>
          ...
          <div className="space-y-2.5 text-[11px] mono-brutal">
            <div className="flex justify-between items-center pb-2 border-b-2 border-[#111111]">
              <span className="text-gray-500 font-bold">STATUS</span>
              <span className="font-bold px-2.5 py-0.5 rounded-full border-2 border-[#111111]"
                style={{ backgroundColor: testResult.ok ? C.success : C.danger, color: testResult.ok ? C.ink : '#fff' }}>
                {testResult.ok ? 'OK' : 'FAIL'}
              </span>
            </div>
            {testResult.error && (
              <div className="pb-2 border-b-2 border-[#111111]">
                <span className="text-gray-500 font-bold">ERROR</span>
            ...
            <div className="flex justify-between"><span className="text-gray-500">Latency</span>
              <span className="text-[#111111] font-bold">{testResult.latency_ms}ms</span></div>
            <div className="flex justify-between"><span className="text-gray-500">Tokens</span>
              <span className="text-[#111111] font-bold">{testResult.total_tokens} (in: {testResult.prompt_tokens}, out: {testResult.completion_tokens})</span></div>
            {testResult.response && (
              <div className="mt-2 p-3 rounded-lg bg-[#f0f0f0] border-2 border-[#111111] text-gray-600 max-h-32 overflow-y-auto">
                <code className="text-[9px]">{testResult.response}</code>
              </div>
            )}
          </div>
          <button onClick={() => ctx.setTestResult(null)} className="brutal-btn w-full mt-4 bg-white text-[#111111] px-4 py-2 text-xs font-bold">Close</button>
        </Modal>
      )}
      ...
      <Modal open={showAddModel} onClose={() => { setShowAddModel(false); setModelError('') }}>
        ...
        <div className="space-y-3">
          <div>
            <label className="text-[10px] mono-brutal text-gray-500 mb-1 block font-bold uppercase">Model ID</label>
            <input value={newModel.model_id} onChange={e => setNewModel(f => ({ ...f, model_id: e.target.value }))}
              placeholder="model-name"
              className="w-full px-3 py-2 border-2 border-[#111111] rounded-lg text-xs mono-brutal bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#ff3d81]" />
          </div>
          <div>
            <label className="text-[10px] mono-brutal text-gray-500 mb-1 block font-bold uppercase">Context Length</label>
            <input type="number" value={newModel.ctx} onChange={e => setNewModel(f => ({ ...f, ctx: +e.target.value }))}
              className="w-full px-3 py-2 border-2 border-[#111111] rounded-lg text-xs mono-brutal bg-white focus:outline-none focus:ring-2 focus:ring-[#ff3d81]" />
          </div>
        </div>
