...
function BrandMark({ size = 'md' }: { size?: 'sm' | 'md' }) {
  ...
  return (
    <img
      src="/logo.png"
      alt="NuvCode"
      className={`${box} rounded-lg border-2 border-[#111111] object-cover shrink-0 bg-white`}
    />
  )
}
...
export default function Layout() {
    ...
    <>
      {sections.map(section => (
        <div key={section.label} className="space-y-1">
          <div className="px-3 mb-3 mono-brutal text-[10px] text-gray-500 uppercase tracking-widest">
            {section.label}
          </div>
            ...
            {section.items.map(n => {
              ...
              return (
                <Link
                  ...
                  to={n.path}
                  onClick={onNavigate}
                  className={`sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl font-bold text-[#111111] ${
                    active ? 'nav-item-active' : 'hover:bg-gray-50'
                  }`}
                >
  ...
  const ProfileCard = () => (
    <div className="brutal-card p-3 flex items-center gap-3">
      <div className="w-10 h-10 bg-[#c8a2ff] border-2 border-[#111111] flex items-center justify-center heading-brutal text-sm shrink-0">
        AD
      </div>
      <div className="flex-1 min-w-0">
        <p className="font-bold text-sm truncate text-[#111111]">Admin</p>
        <div className="inline-block px-2 py-0.5 bg-[#ffd23f] border border-[#111111] text-[9px] font-black uppercase rounded">
          Enterprise
        </div>
      </div>
    </div>
  ...
  const LogoutBtn = ({ full = false }: { full?: boolean }) => (
    <button
      ...
      className="brutal-btn w-full bg-white px-3 py-2 mt-3 text-[12px] font-bold text-[#111111]"
    >
  ...
  return (
    <div className="min-h-screen bg-[#fdf9f0] text-[#111111]">
      <div className="grid-pattern" />

      {/* === DESKTOP SIDEBAR === */}
      <aside className="hidden lg:flex fixed inset-y-0 left-0 w-[240px] flex-col bg-white border-r-2 border-[#111111] z-40">
        <div className="p-6 flex items-center gap-3">
          <BrandMark />
          <span className="heading-brutal text-xl text-[#111111]">NUVCODE</span>
        </div>
      ...
      {/* === MOBILE HEADER === */}
      <header className="lg:hidden fixed top-0 inset-x-0 z-40 h-16 px-4 flex items-center justify-between bg-white border-b-2 border-[#111111]">
        <div className="flex items-center gap-2.5 min-w-0">
          <button
            onClick={() => setMobileOpen(true)}
            className="brutal-btn w-9 h-9 bg-white !p-0"
          ...
          <span className="heading-brutal text-sm truncate text-[#111111]">NUVCODE</span>
        </div>
      ...
      {mobileOpen && (
        <div
          className="lg:hidden fixed inset-0 z-50 bg-[#111111]/60"
      ...
      {/* === MOBILE DRAWER === */}
      <aside
        className={`lg:hidden fixed top-0 left-0 z-50 h-full w-[17.5rem] max-w-[86vw] bg-white border-r-2 border-[#111111] shadow-[4px_0_0px_0px_#111111] sm:shadow-[6px_0_0px_0px_#111111] lg:shadow-[8px_0_0px_0px_#111111] transform transition-transform duration-200 ease-out flex flex-col ${
          mobileOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex items-center justify-between px-4 h-16 border-b-2 border-[#111111] shrink-0">
          <div className="flex items-center gap-2.5 min-w-0">
            <BrandMark size="sm" />
            <span className="heading-brutal text-sm text-[#111111]">NUVCODE</span>
          </div>
          <button
            onClick={() => setMobileOpen(false)}
            className="brutal-btn w-9 h-9 bg-white !p-0"
        ...
        <div className="shrink-0 p-4 border-t-2 border-[#111111] bg-white">
          <ProfileCard />
          <LogoutBtn full />
        </div>
      </aside>
