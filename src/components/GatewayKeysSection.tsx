...
export default function GatewayKeysSection({ keys, onRefresh }: Props) {
  ...
  return (
    <div className="brutal-card overflow-hidden">
      <div className="px-5 py-4 border-b-2 border-[#111111] flex items-center justify-between">
        <div>
          <h2 className="heading-brutal text-lg uppercase tracking-tight">GATEWAY KEYS</h2>
          <p className="text-xs text-gray-600 mt-0.5">{keys.length} key{keys.length !== 1 ? 's' : ''} · API access tokens</p>
        </div>
        {keys.length > 0 && (
          <span className="text-xs font-mono text-gray-600">{keys.filter(k => k.is_active).length} active</span>
        )}
      </div>

      {/* Create form */}
      <div className="p-5 border-b-2 border-[#111111] bg-gray-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3">
          <div>
            <div className="text-[9px] font-mono text-gray-500 mb-1.5 uppercase tracking-wide">Label</div>
            <input type="text" value={label} onChange={e => setLabel(e.target.value)} placeholder="e.g. production"
              className="w-full border-2 border-[#111111] rounded-lg px-3 py-2.5 text-[11px] font-mono placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#ff3d81]" />
          </div>
          <div>
            <div className="text-[9px] font-mono text-gray-500 mb-1.5 uppercase tracking-wide">Access Type</div>
            <div className="flex bg-white rounded-lg p-0.5 border-2 border-[#111111]">
              {['full','allow','deny'].map(t => (
                <button key={t} onClick={() => setAccessType(t)}
                  className={`flex-1 py-2 rounded-md text-[10px] font-mono font-semibold transition-all ${
                    accessType === t
                      ? t === 'full' ? 'bg-[#3ddc97] text-white'
                      : t === 'allow' ? 'bg-[#c8a2ff] text-white'
          ...
          {accessType !== 'full' ? (
            <div>
              <div className="text-[9px] font-mono text-gray-500 mb-1.5 uppercase tracking-wide">Models</div>
              <button onClick={() => { apiFetch('/models/all').then(r => r.json()).then(data => { setAllModels(data); setShowPicker(true) }).catch(() => {}) }}
                className="w-full border-2 border-[#111111] rounded-lg px-3 py-2.5 text-[11px] font-mono text-left hover:border-[#ff3d81] transition-all bg-white">
                {modelList.length === 0 ? <span className="text-gray-400">Select models...</span> : <span className="text-[#ff3d81]">{modelList.length} selected</span>}
              </button>
              {modelList.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  ...
                  {modelList.length > 3 && <span className="text-[9px] font-mono text-gray-500">+{modelList.length-3} more</span>}
                </div>
              )}
            </div>
          ) : (
            <div>
              <div className="text-[9px] font-mono text-gray-500 mb-1.5 uppercase tracking-wide">Max Tokens</div>
        ...
        {copied && (
          <div className="mt-3 p-4 brutal-card bg-[#3ddc97]/10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-mono font-bold text-[#111111] tracking-wider uppercase">KEY GENERATED — COPY & SAVE NOW</span>
            </div>
            <code className="text-xs font-mono text-[#111111] bg-white rounded-lg p-3 block break-all border-2 border-[#111111]">{copied}</code>
            <p className="text-[9px] font-mono text-gray-600 mt-2">This key won't be shown again.</p>
          </div>
        )}
      </div>
      ...
      <div className="divide-y divide-gray-200">
        {keys.map(k => {
          ...
          return (
            <div key={k.id} className="px-5 py-4 hover:bg-gray-50 transition-all group">
              <div className="flex items-start gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2.5 mb-2 flex-wrap">
                    <div className={`w-2 h-2 rounded-full shrink-0 border border-[#111111] ${k.is_active ? 'bg-[#3ddc97]' : 'bg-gray-300'}`} />
                    <span className="text-sm font-mono font-semibold text-[#111111]">{k.label || 'unnamed'}</span>
                    <span className={`text-[9px] font-mono px-2 py-0.5 rounded-full border-2 border-[#111111] text-white ${
                      k.access_type === 'full' ? 'bg-[#3ddc97]' :
                      k.access_type === 'allow' ? 'bg-[#c8a2ff]' :
                      'bg-[#ff6b5e]'
                    }`}>
                    ...
                    {k.max_tokens > 0 && (
                      <span className="text-[9px] font-mono text-gray-600 bg-gray-100 px-2 py-0.5 rounded-full border border-gray-300">
                  ...
                  <code className="text-[11px] font-mono text-gray-600 block mb-1.5 bg-gray-100 rounded-lg px-3 py-2 border-2 border-[#111111] break-all max-w-full overflow-x-hidden">
                    {k.key_value}
                  </code>
                  ...
                  {editingKey === k.id && (
                    <div className="mt-3 p-4 brutal-card bg-white">
                      <div className="text-[9px] font-mono text-[#ff3d81] uppercase tracking-wide mb-3">Edit Key</div>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                        <div>
                          <div className="text-[8px] font-mono text-gray-500 mb-1 uppercase">Label</div>
                          <input type="text" value={editLabel} onChange={e => setEditLabel(e.target.value)} placeholder={k.label || 'Label...'}
                            className="w-full border-2 border-[#111111] rounded-lg px-3 py-2 text-[11px] font-mono placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#ff3d81]" autoFocus />
                        </div>
                        <div>
                          <div className="text-[8px] font-mono text-gray-500 mb-1 uppercase">Access</div>
                          <div className="flex bg-white rounded-lg p-0.5 border-2 border-[#111111]">
                            {['full','allow','deny'].map(t => (
                              <button key={t} onClick={() => setEditAccess(t)}
                                className={`flex-1 py-1.5 rounded text-[9px] font-mono font-semibold transition-all ${
                                  editAccess === t
                                    ? t === 'full' ? 'bg-[#3ddc97] text-white' : t === 'allow' ? 'bg-[#c8a2ff] text-white' : 'bg-[#ff6b5e] text-white'
                                    : 'text-gray-500 hover:text-gray-700'
                                }`}>{t === 'full' ? 'Full' : t === 'allow' ? 'Allow' : 'Deny'}</button>
                            ))}
                          </div>
                        ...
                        <div>
                          <div className="text-[8px] font-mono text-gray-500 mb-1 uppercase">Max Tokens</div>
                          <input type="number" value={editMaxTokens} onChange={e => setEditMaxTokens(e.target.value)} placeholder="0 = unlimited"
                            className="w-full border-2 border-[#111111] rounded-lg px-3 py-2 text-[11px] font-mono placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#ff3d81]" />
                        </div>
                      </div>
                      {editAccess !== 'full' && (
                        <div className="mb-3">
                          <div className="text-[8px] font-mono text-gray-500 mb-1 uppercase">Models</div>
                          <button onClick={() => { apiFetch('/models/all').then(r => r.json()).then(data => { setEditAllModels(data); setEditShowPicker(true) }).catch(() => {}) }}
                            className="w-full border-2 border-[#111111] rounded-lg px-3 py-2 text-[11px] font-mono text-left hover:border-[#ff3d81] transition-all bg-white">
                            {editModelList.length === 0 ? <span className="text-gray-400">Select models...</span> : <span className="text-[#ff3d81]">{editModelList.length} selected</span>}
                          </button>
                          {editModelList.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-2">
                              ...
                              {editModelList.length > 3 && <span className="text-[9px] font-mono text-gray-500">+{editModelList.length-3} more</span>}
                            </div>
                          )}
                        </div>
                      )}
                      <div className="flex items-center justify-between">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={editActive} onChange={e => setEditActive(e.target.checked)}
                            className="w-3.5 h-3.5 rounded border-2 border-[#111111] accent-[#ff3d81]" />
                          <span className="text-[9px] font-mono text-gray-600">Active</span>
                        </label>
                        <div className="flex items-center gap-2">
                          <button onClick={() => setEditingKey('')} className="px-3 py-2 rounded-lg text-[10px] font-mono text-gray-500 hover:text-gray-700 transition-all">Cancel</button>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => { setEditingKey(k.id === editingKey ? '' : k.id); setEditLabel(k.label || ''); setEditAccess(k.access_type || 'full'); setEditMaxTokens(k.max_tokens > 0 ? String(k.max_tokens) : ''); setEditActive(k.is_active === 1); setEditModelList(k.allowed_models || []) }}
                    className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all shrink-0 border-2 ${
                      k.id === editingKey ? 'text-[#ff3d81] bg-[#ff3d81]/10 border-[#ff3d81]' : 'text-gray-400 hover:text-[#ff3d81] hover:bg-[#ff3d81]/10 border-transparent hover:border-[#ff3d81]'
                  ...
                  <button onClick={() => deleteKey(k.id)}
                    className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-[#ff6b5e] hover:bg-[#ff6b5e]/10 transition-all shrink-0 border-2 border-transparent hover:border-[#ff6b5e] opacity-50 group-hover:opacity-100"
                    title="Delete key">
        ...
        {keys.length === 0 && (
          <div className="px-5 py-16 text-center">
            <div className="w-12 h-12 mx-auto mb-3 rounded-xl bg-[#ff3d81]/10 border-2 border-[#111111] flex items-center justify-center">
            ...
            <div className="text-xs font-mono font-semibold text-gray-600 mb-1">No Gateway Keys</div>
            <p className="text-[10px] font-mono text-gray-500">Generate one above to access the API</p>
          </div>
        )}
      </div>
